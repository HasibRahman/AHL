...
*        REFRESH it_podet.          " Commented on 03.07.2022
      ENDIF.
*      IF s_ebeln IS NOT INITIAL.   " Commented on 03.07.2022
*        PERFORM search_pi.
...
MODULE user_command_9001 INPUT.
  ...
  CASE sy-ucomm.
    ...
    WHEN 'LC_TRACK_REP'.
      CALL SCREEN '9007'.
    "BOC by Hasib on 17.10.2022
    WHEN 'LANDED_COST'.
      CALL TRANSACTION 'ZLC_COST'.
      "EOC"
  ENDCASE.
...
"BOC  By Hasib on 03.07.2022
  READ TABLE it_podet INTO DATA(wa_pod1) WITH KEY pi  =  wa_podet-pi
                                                    ebeln = wa_podet-ebeln
                                                    ebelp = wa_podet-ebelp.
  IF sy-subrc = 0.
    MODIFY it_podet FROM wa_podet INDEX sy-tabix .
  ENDIF.
"EOC by Hasib
