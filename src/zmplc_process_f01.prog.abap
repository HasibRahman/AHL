...
*  ENDIF.                                                    "--
  ...
  gs_1004-irc_no = '260326120460120'.
  gs_1004-bin_no = '001299707-0304'.
  gs_1004-tin_no = '227666490808'.
  "EOC NKAUL 17.06.2021
...
*  PERFORM fill_po.
  ...
  IF gv_screenmode = 'PI_MOD'.
    "BOC by EY_ABAP ( Hasib)
    LOOP AT it_podet1 ASSIGNING FIELD-SYMBOL(<fs_pod>).
      MOVE-CORRESPONDING <fs_pod> TO gs_1001.
...
*      MOVE-CORRESPONDING gs_1001 TO <gs_1001>.
      ...
      "BOC NKAUL 17.06.2021
      <gs_1001>-irc_no = '260326120460120'.
      <gs_1001>-bin_no = '001299707-0304'.
      <gs_1001>-tin_no = '227666490808'.
      "EOC NKAUL 17.06.2021
      <gs_1001>-trans_ship = gs_1001-trans_ship.  "++NKAUL
      <gs_1001>-part_ship = gs_1001-part_ship.    "++NKAUL
      <gs_1001>-iban_no = gs_1001-iban_no.        "++NKAUL
      <gs_1001>-auth_indent = gs_1001-auth_indent."++NKAUL
      <gs_1001>-bb_perms_no = gs_1001-bb_perms_no."++NKAUL
    ENDLOOP.
  ...
  LOOP AT gt_1001 ASSIGNING FIELD-SYMBOL(<fs_1001>).
    ...
    "BOC NKAUL 17.06.2021
    <fs_1001>-irc_no = '260326120460120'.
    <fs_1001>-bin_no = '001299707-0304'.
    <fs_1001>-tin_no = '227666490808'.
    "EOC NKAUL 17.06.2021
...
FORM fill_po .
  ...
  IF lt_ekko IS NOT INITIAL.
    ...
    IF lt_ekpo IS NOT INITIAL.
      "BOC Nkaul 14.06.2021
      DATA(lt_tmp1) = lt_ekpo.
      SORT lt_tmp1 BY matnr.
      DELETE ADJACENT DUPLICATES FROM lt_tmp1 COMPARING matnr.
      SELECT matnr, mfrnr
        FROM mara
        INTO TABLE @DATA(lt_mrfnr)
        FOR ALL ENTRIES IN @lt_tmp1
        WHERE matnr = @lt_tmp1-matnr.
      IF sy-subrc EQ 0.
        SORT lt_mrfnr BY matnr.
      ENDIF.
      "EOC Nkaul 14.06.2021
...
*  gs_1001-pi_pymt = ls_ekko-zterm.
  ...
  IF lt_ser IS INITIAL.
    LOOP AT lt_ekpo INTO DATA(ls_ekpo).
      "BOC Nkaul 14.06.2021
      READ TABLE gt_1001 ASSIGNING FIELD-SYMBOL(<fs_tmp_1001>) WITH KEY ebeln = ls_ekpo-ebeln
                                                                        ebelp = ls_ekpo-ebelp.
      IF sy-subrc EQ 0.
        wa_podet-fob_freight = <fs_tmp_1001>-fob_freight.
      ENDIF.
      READ TABLE lt_mrfnr ASSIGNING FIELD-SYMBOL(<fs_mrfnr>) WITH KEY matnr = ls_ekpo-matnr BINARY SEARCH.
      IF sy-subrc EQ 0.
        wa_podet-manuf_name = <fs_mrfnr>-mfrnr.
      ENDIF.
      "EOC Nkaul 14.06.2021
    ...
    IF gv_screenmode = 'INS_CRT'.
      LOOP AT gt_1003 ASSIGNING FIELD-SYMBOL(<fs_1003>).
        IF <fs_1003>-ins_vendor IS NOT INITIAL.
          MESSAGE 'Insurance Information already created for PI' TYPE 'I' ##NO_TEXT.""++ATC Fix By EY_SANTOSH(Pravas) On 25/08/2021
        ENDIF.
      ENDLOOP.
    ENDIF.
...
FORM lc_report .
  ...
  SELECT ebeln, ebelp, pi_no, pi_dt, pi_amt, lifnr, waers, inco, lc_val, lc_no, lc_date, ship_date, lc_exp_date, lstel, vstel
    FROM zlc_process
  ...
  IF gt_lc IS NOT INITIAL.
    SELECT ebeln,ebelp, matnr, meins, menge FROM ekpo INTO TABLE @DATA(lt_ekpo)
...
*    READ TABLE lt_ekpo INTO DATA(ls_ekpo) WITH KEY ebeln = gs_lc-ebeln .
    READ TABLE lt_ekpo INTO DATA(ls_ekpo) WITH KEY ebeln = gs_lc-ebeln ebelp = gs_lc-ebelp. " Added on 23.06.2022 by Hasib (EY_SANTOSH)
    IF sy-subrc = 0.
...
*--------Preparing Field Catalogue------------->
  ...
  wa_fcat-seltext_m  = 'Proforma Invoice No' ##NO_TEXT.
  APPEND wa_fcat TO it_fcat.
  CLEAR wa_fcat.

  wa_fcat-fieldname  = 'PI_DT'.
  wa_fcat-outputlen  = '20' .
  wa_fcat-seltext_m  = 'Proforma Inv Date' ##NO_TEXT.
  APPEND wa_fcat TO it_fcat.
  CLEAR wa_fcat.

  wa_fcat-fieldname  = 'PI_AMT'.
  wa_fcat-outputlen  = '20' .
  wa_fcat-seltext_m  = 'Proforma Inv Amount' ##NO_TEXT.
  APPEND wa_fcat TO it_fcat.
  CLEAR wa_fcat.

  wa_fcat-fieldname  = 'LIFNR'.
  wa_fcat-outputlen  = '20' .
  wa_fcat-seltext_m  = 'Vendor/Supplier' ##NO_TEXT.
  APPEND wa_fcat TO it_fcat.
  CLEAR wa_fcat.

  wa_fcat-fieldname  = 'MATNR'.
  wa_fcat-outputlen  = '15' .
  wa_fcat-seltext_m  = 'Material Code' ##NO_TEXT.
  APPEND wa_fcat TO it_fcat.
  CLEAR wa_fcat.

  wa_fcat-fieldname  = 'MAKTX'.
  wa_fcat-outputlen  = '20' .
  wa_fcat-seltext_m  = 'Material Description' ##NO_TEXT.
  APPEND wa_fcat TO it_fcat.
  CLEAR wa_fcat.

  wa_fcat-fieldname  = 'MENGE'.
  wa_fcat-outputlen  = '15' .
  wa_fcat-seltext_m  = 'Qty' ##NO_TEXT.
  APPEND wa_fcat TO it_fcat.
  CLEAR wa_fcat.

  wa_fcat-fieldname  = 'MEINS'.
  wa_fcat-outputlen  = '15' .
  wa_fcat-seltext_m  = 'Unit' ##NO_TEXT.
  APPEND wa_fcat TO it_fcat.
  CLEAR wa_fcat.

  wa_fcat-fieldname  = 'LC_VAL'.
  wa_fcat-outputlen  = '20' .
  wa_fcat-seltext_m  = 'LC Value' ##NO_TEXT.
  APPEND wa_fcat TO it_fcat.
  CLEAR wa_fcat.

  wa_fcat-fieldname  = 'WAERS'.
  wa_fcat-outputlen  = '10' .
  wa_fcat-seltext_m  = 'Currency' ##NO_TEXT.
  APPEND wa_fcat TO it_fcat.
  CLEAR wa_fcat.

  wa_fcat-fieldname  = 'INCO'.
  wa_fcat-outputlen  = '15' .
  wa_fcat-seltext_m  = 'Incoterms' ##NO_TEXT.
  APPEND wa_fcat TO it_fcat.
  CLEAR wa_fcat.

  wa_fcat-fieldname  = 'LC_NO'.
  wa_fcat-outputlen  = '20' .
  wa_fcat-seltext_m  = 'LC No' ##NO_TEXT.
  APPEND wa_fcat TO it_fcat.
  CLEAR wa_fcat.

  wa_fcat-fieldname  = 'LC_DATE'.
  wa_fcat-outputlen  = '20' .
  wa_fcat-seltext_m  = 'LC Date' ##NO_TEXT.
  APPEND wa_fcat TO it_fcat.
  CLEAR wa_fcat.

  wa_fcat-fieldname  = 'SHIP_DATE'.
  wa_fcat-outputlen  = '20' .
  wa_fcat-seltext_m  = 'Latest Shipment Date' ##NO_TEXT.
  APPEND wa_fcat TO it_fcat.
  CLEAR wa_fcat.

  wa_fcat-fieldname  = 'LC_EXP_DATE'.
  wa_fcat-outputlen  = '20' .
  wa_fcat-seltext_m  = 'LC Expiry Date' ##NO_TEXT.
  APPEND wa_fcat TO it_fcat.
  CLEAR wa_fcat.

  wa_fcat-fieldname  = 'LSTEL'.
  wa_fcat-outputlen  = '30' .
*  wa_fcat-seltext_s = wa_fcat-seltext_m  =
  wa_fcat-seltext_l = 'Loading Port' ##NO_TEXT.
  APPEND wa_fcat TO it_fcat.
  CLEAR wa_fcat.
  ...
  wa_fcat-outputlen  = '30' .
*  wa_fcat-seltext_s = wa_fcat-seltext_m  =
  wa_fcat-seltext_l = 'Destination Port' ##NO_TEXT.
  APPEND wa_fcat TO it_fcat.
  CLEAR wa_fcat.
...
* Implement suitable error handling here
    ...
    MODIFY zins_policy FROM wa_zins ##WARN_OK. ""++ATC Fix By EY_SANTOSH(Pravas) On 25/08/2021
    IF sy-subrc = 0.
      COMMIT WORK AND WAIT.
    ELSE.
      ROLLBACK WORK.
...
FORM close_lc .
  ...
  CALL TRANSACTION 'ZLC_CLOSE' ##TSTC_MISS.""++ATC Fix By EY_SANTOSH(Pravas) On 25/08/2021.
ENDFORM.
*&---------------------------------------------------------------------*
*& Form get_retirement
*&---------------------------------------------------------------------*
...
FORM end_copy_doc_n USING lv_i TYPE i.
  ...
  CLEAR: lt_proc1,
         wa_proc.
  lt_proc1 = lt_proc.
  SORT lt_proc1 BY vstel.
  DELETE ADJACENT DUPLICATES FROM lt_proc1 COMPARING vstel.

  LOOP AT lt_proc1 INTO wa_proc.
    lv_ind = lv_ind + 1.
    IF lv_ind = 1.
...
*          gw_header-subject = 'Endorsement of copy documents against LC/CAD (LCAF)'.
          gw_header-subject = 'Request for issuance of Shipping Guarantee (SG)'.
...
*          'instruction. We hereby confirm you that we will accept the original shipping documents irrespective of discrepancy (if any) .'
           gw_header-line1 = 'We would like to request you to issue an SG to release the consignment for the enclosed non-negotiable shipping documents as referred information in the subject.'.
*          CONCATENATE TEXT-t00 TEXT-t01 INTO gw_header-line2 SEPARATED BY space.
          CONCATENATE
          'Please adjust the invoiced value and other charges (if any) with our appropriate account creating short/ long term loan or LATR or on maturity as per LC terms and condition.'
          'We would like to assure you that we will accept the discrepancies if the original document bears any.'
...
*          gw_header-line3 = 'Please note that in case of payment against CAD (LCAF), it should be made only after submission of Bill of Entry at your bank counter.'.
          gw_header-line3 = 'NB.: If the original document reaches your good office before this SG request, please consider this letter sufficient enough to retire the document.'.
...
*          gw_header-line3 = 'Please note that in case of payment against CAD (LCAF), it should be made only after submission of Bill of Entry at your bank counter.'.
          gw_header-subject = 'Request for retirement of original document'.
          gw_header-line1 = 'We would like to request you to retire the original shipping documents to release the consignment for the referred information in the subject.'.
          gw_header-line2 = ' Please adjust the invoiced value and other charges (if any) with our appropriate account creating short/ long term loan or LATR or on maturity as per LC terms and condition.'.
*          gw_header-line3 = 'Please note that in case of payment against CAD (LCAF), it should be made only after submission of Bill of Entry at your bank counter.'.
...
*          gw_header-desig2 = 'Managing Director'.
  ...
  MODIFY zendrs_ltr FROM lw_endi ##WARN_OK.""++ATC Fix By EY_SANTOSH(Pravas) On 25/08/2021
  IF sy-subrc EQ 0.
    COMMIT WORK AND WAIT.
  ELSE.
    ROLLBACK WORK.
